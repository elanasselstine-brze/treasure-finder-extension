// Content script for UX Issue Tracker
...
class UXTrackerContent {
    ...
    createSelectionOverlay() {
        ...
        instructions.innerHTML = `
            <div style="font-weight: 600; margin-bottom: 8px;">Select Problem Area</div>
            <div>Click and drag to highlight the specific area with the issue</div>
            <div style="margin-top: 8px; font-size: 12px; color: #666;">Press ESC to cancel</div>
        `;
    ...
    completeSelection() {
        ...
        // Send selection data to background script (popup is closed)
        console.log('📤 CONTENT: Sending areaSelected message to background:', selection);
        ...
        // Remove overlay after brief delay to show completion
        setTimeout(() => {
            this.removeSelectionOverlay();
        }, 300);
    }
    ...
    handleKeyDown(event) {
        if (event.key === 'Escape' && this.selectionOverlay) {
            this.cancelAreaSelection();
        }
    }
