// JIRA Integration Module for UX Issue Tracker
...
class JIRAIntegration {
    ...
    async buildJIRAPayload(issueData) {
        ...
        try {
            ...
            console.warn('⚠️ This is likely a JIRA permissions issue - you need "View Development Tools" or "Administer Projects" permission');
            console.warn('⚠️ Falling back to common issue types...');
            
            // Try common issue types in order of likelihood
            const commonFallbacks = [
                this.issueType,    // User's configured type first
                'Task',           // Most common in JIRA Cloud
                'Story',          // Common in Agile projects  
                'Bug',            // Common bug tracking
                'Improvement',    // Common enhancement type
                'Epic'            // Common high-level type
            ];
            
            // Use the first valid-looking type
            issueTypeToUse = commonFallbacks[0];
            console.log(`🎯 Using fallback issue type: ${issueTypeToUse}`);
            console.log('💡 If JIRA still rejects this, ask your admin which issue types are available in your project');
        }
    ...
    async debugProjectInfo() {
        ...
        try {
            ...
            try {
                ...
                console.error('❌ Failed to get issue types:', issueTypesError.message);
                console.warn('⚠️ This usually means you need "View Development Tools" or "Administer Projects" permission');
                console.log('💡 Common workaround: Try these issue types manually: Task, Story, Bug, Improvement');
                
                // Don't throw error - provide guidance instead
                issueTypes = [
                    { name: 'Task', id: 'unknown-task' },
                    { name: 'Story', id: 'unknown-story' }, 
                    { name: 'Bug', id: 'unknown-bug' },
                    { name: 'Improvement', id: 'unknown-improvement' }
                ];
                console.log('📝 Using common fallback issue types for testing');
            }
            console.log('');
    ...
    async testConnection() {
        ...
        try {
            ...
            try {
                ...
                console.warn('⚠️ Could not verify detailed permissions, but basic access works');
                console.log('  Issue types error (non-critical):', permError.message);
            }
